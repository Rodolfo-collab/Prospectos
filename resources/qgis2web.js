...
var map = new ol.Map({
    ...
    view: new ol.View({
         maxZoom: 28, minZoom: 1, projection: new ol.proj.Projection({
            code: 'EPSG:32614',
            //extent: [352476.200000, 2015415.800000, 643238.400000, 2336168.700000],
            units: 'm'})
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([150406.447532, 1936195.530791, 1007240.817491, 2395368.105624], map.getSize());
            ...
            if (layer.get('fieldImages')[currentFeatureKeys[i]] != "ExternalResource") {
				...
				if (/\.(gif|jpg|jpeg|tif|tiff|png|avif|webp|svg)$/i.test(fieldValue)) {
                    ...
                    popupField += (fieldValue != null ? '<audio controls><source src="images/' + fieldValue.replace(/[\\\/:]/g, '_').trim() + '" type="audio/mpeg">Il tuo browser non supporta il tag audio.</audio></td>' : '');
					   ...
					   if (/\.pdf$/i.test(fieldValue)) {

        popupField += (fieldValue != null ?

        '<a class="pdf-button" href="' + fieldValue + '" target="_blank">' +
